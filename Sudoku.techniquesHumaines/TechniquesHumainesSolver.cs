using Sudoku.TechniquesHumaines;
using System;
...
namespace Sudoku.techniquesHumaines
{
    ...
    public class TechniquesHumainesSolver : Sudoku.Core.ISudokuSolver
    {
        ...
        public Core.Sudoku transformationToSudoku(Puzzle puzzle)
        {
            ...
            for (int i = 0; i<9; i++)
            {
                for(int j = 0; j<9; j++)
                {
                    listCells.Add(puzzle[i, j].Value);
                }
            }
            Core.Sudoku sudoku = new Core.Sudoku(listCells);
            return sudoku;
        }
        ...
        bool RunTechnique(Puzzle puzzle)
        {
